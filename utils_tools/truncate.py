...
def truncate_preserve_tokens(s: str, limit: int, tokens: List[str], case_ins: bool) -> str:
    ...
    while calc_len(cur) > limit:
        ...
        if run_len > 0:
            ...
            if len(cur) <= run_len:
                ...
                raise ValueError("无法在不删除保护 token 的前提下继续截断（字符串仅剩保护 token）。")
...
def process_all(raw_list: list, trans_list: list) -> list:
    if len(raw_list) != len(trans_list):
        raise ValueError(f"长度不一致：原文 {len(raw_list)} 项，译文 {len(trans_list)} 项。")
    out = []
    for idx, (o, t) in enumerate(zip(raw_list, trans_list)):
        ...
        for key in ("name", "message"):
            if key in o and key not in t:
                raise ValueError(f"第 {idx} 项结构不一致：原文有字段 '{key}'，译文缺失。")
        new_t = dict(t)
